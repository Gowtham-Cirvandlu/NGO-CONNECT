...
from database.models import db, User, NGO, Volunteer, Donor, Event, TimeSlot, Booking, Message, Resource, Project, Donation, AdminAuditLog, AdminRole, AdminUserRole, AttendanceToken
...
def about():
    try:
        ...
        active_events = Event.query.filter_by(is_active=True).count() if hasattr(Event, 'is_active') else events_count
        confirmed_donations_q = Donation.query.filter_by(status='confirmed') if hasattr(Donation, 'status') else Donation.query
        total_donations = confirmed_donations_q.count()
        total_donated = sum((d.amount_inr or 0) for d in confirmed_donations_q.all())
        lives_impacted = int((total_donated or 0) * 0.03)
        ...
        stats = [
            {'value': short(ngos_count), 'label': 'Active NGOs'},
            {'value': short(volunteers_count), 'label': 'Volunteers'},
            {'value': short(active_events), 'label': 'Active Events'},
            {'value': short(total_donations), 'label': 'Donations'},
            {'value': short(lives_impacted), 'label': 'Lives Impacted'},
        ]

        timeline = [
            {'year': '2020', 'title': 'Foundation', 'desc': 'NGO Hub was founded with a mission to bridge communities and causes.'},
            {'year': '2021', 'title': 'Growth', 'desc': 'Reached 100 NGOs and 1,000 volunteers committed to positive change.'},
            {'year': '2023', 'title': 'Expansion', 'desc': 'Expanded to multiple cities with thousands of active volunteers.'},
            {'year': str(datetime.now(UTC).year), 'title': 'Impact', 'desc': f"Serving {ngos_count}+ NGOs and creating measurable social impact."},
        ]

        return render_template('about.html', stats=stats, timeline=timeline)
    except Exception as e:
        # Fallback to template with minimal context if something goes wrong
        return render_template('about.html', stats=[], timeline=[])
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            login_user(user)
            # Store naive IST timestamp for DB consistency
            user.last_login = datetime.now(ZoneInfo("Asia/Kolkata")).replace(tzinfo=None)
            db.session.commit()
...
def volunteer_dashboard():
    ...
    # Get volunteer statistics and data
    stats = queries.get_volunteer_stats(volunteer.id)
    recommended_events = queries.get_recommended_events(volunteer.id, 5)

    # Build recent bookings with joined Event/NGO data for display
    try:
        # Include TimeSlot if schema uses slot-specific booking
        q = (
            db.session.query(Booking, Event, NGO, TimeSlot)
            .join(Event, Booking.event_id == Event.id, isouter=True)
            .join(NGO, Event.ngo_id == NGO.id, isouter=True)
            .join(TimeSlot, Booking.time_slot_id == TimeSlot.id, isouter=True)
            .filter(Booking.volunteer_id == volunteer.id)
        ...
        def _fmt_dt(dt):
            try:
                ...
                return ''
        bookings = []
        # Use IST naive current time for comparisons
        now_ts = datetime.now(ZoneInfo("Asia/Kolkata")).replace(tzinfo=None)
        for b, e, n, ts in rows:
            # Title fallbacks: Event.title -> Event.name -> Booking.title/id
            evt_title = (getattr(e, 'title', None) or getattr(e, 'name', None) 
                         or getattr(b, 'title', None) or f"Event #{getattr(e, 'id', '') or getattr(b, 'event_id', '')}")
            # Date fallbacks: Event.start_date -> Booking.created_at
            when_dt = (getattr(e, 'start_date', None) or getattr(ts, 'start_time', None)
                       or getattr(b, 'start_time', None) or getattr(b, 'created_at', None))
            # Location fallbacks: Event.location -> NGO city/state -> Booking.location
            loc = (getattr(e, 'location', None) or getattr(ts, 'location', None) or getattr(b, 'location', None))
            if not loc:
                city = getattr(n, 'city', None) or ''
                state = getattr(n, 'state', None) or ''
                loc = (city + ((', ' + state) if city and state else '')).strip() or '-'
            # Effective status for UI: 'not_completed' if the time slot or event has ended without attendance
            base_status = getattr(b, 'status', None) or 'pending'
            _ts_end = getattr(ts, 'end_time', None)
            if _ts_end is not None:
                try:
                    _ts_end = _ts_end.replace(tzinfo=None)
                except Exception:
                    pass
            slot_over = bool(_ts_end and _ts_end <= now_ts)
            _ev_end = getattr(e, 'end_date', None)
            if _ev_end is not None:
                try:
                    _ev_end = _ev_end.replace(tzinfo=None)
                except Exception:
                    pass
            event_over = bool(_ev_end and _ev_end <= now_ts)
            # If booking is already completed, keep it. Otherwise, if time is over, mark as not_completed for display
            eff_status = base_status
            if base_status != 'completed' and (slot_over or event_over):
                eff_status = 'not_completed'
            bookings.append({
                'event_id': getattr(e, 'id', None) or getattr(b, 'event_id', None),
                'title': evt_title,
                'event_title': evt_title,
                'ngo_name': getattr(n, 'organization_name', None) or '-',
                'when': _fmt_dt(when_dt),
                'start_at': _fmt_dt(when_dt),
                'location': loc,
                'status': eff_status,
                'hours': getattr(b, 'hours_worked', 0) or 0,
                'points': getattr(b, 'points_earned', 0) or 0,
                'id': getattr(e, 'id', None) or getattr(b, 'event_id', None),
            })
    except Exception:
        # Fallback: build enriched bookings without complex joins
        try:
            rows = (Booking.query.filter_by(volunteer_id=volunteer.id)
            ...
            now_ts = datetime.now(ZoneInfo("Asia/Kolkata")).replace(tzinfo=None)
            for b in rows:
                e = db.session.get(Event, getattr(b, 'event_id', None))
                n = db.session.get(NGO, getattr(e, 'ngo_id', None)) if e else None
                ts = db.session.get(TimeSlot, getattr(b, 'time_slot_id', None)) if getattr(b, 'time_slot_id', None) else None
                evt_title = (getattr(e, 'title', None) or getattr(e, 'name', None)
                             or getattr(b, 'title', None) or f"Event #{getattr(e, 'id', '') or getattr(b, 'event_id', '')}")
                when_dt = (getattr(e, 'start_date', None) or getattr(ts, 'start_time', None)
                           or getattr(b, 'start_time', None) or getattr(b, 'created_at', None))
...
def volunteer_achievements():
    ...
    stats = queries.get_volunteer_stats(volunteer.id)
    # Build recent bookings (joined) so template fields are populated
    try:
        q = (
            db.session.query(Booking, Event, NGO, TimeSlot)
        ...
        for b, e, n, ts in rows:
            ...
            if not loc:
                ...
                loc = (city + ((', ' + state) if city and state else '')).strip() or '-'
            base_status = getattr(b, 'status', None) or 'pending'
            slot_over = bool(getattr(ts, 'end_time', None) and getattr(ts, 'end_time') < now)
            event_over = bool(getattr(e, 'end_date', None) and getattr(e, 'end_date') < now)
            eff_status = 'ended' if (slot_over or event_over) else base_status
        ...
        # Fallback: enriched recent bookings without joins
        try:
            rows = (Booking.query.filter_by(volunteer_id=volunteer.id)
...
def admin_set_donation_status(donation_id: int):
    ...
    try:
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({ 'success': True, 'id': d.id, 'status': d.status })
...
def ngos_directory():
    ...
    query = NGO.query
    # Multi-token free text search across name, description, city, and category
    if search_term:
        tokens = [t for t in (search_term or '').split() if t]
        for tok in tokens:
            like = f"%{tok}%"
            query = query.filter(
                or_(
                    NGO.organization_name.ilike(like),
                    NGO.description.ilike(like),
                    NGO.city.ilike(like),
                    NGO.category.ilike(like)
                )
            )
    # Category typed filter (partial match)
    if category:
        query = query.filter(NGO.category.ilike(f"%{category}%"))
    # City typed filter (partial match across city and state)
    if city:
        like_city = f"%{city}%"
        # If state column exists, include it; otherwise this will just apply to city
        try:
            from database.models import NGO as _NGO
            has_state = hasattr(_NGO, 'state')
        except Exception:
            has_state = False
        if has_state:
            query = query.filter(or_(NGO.city.ilike(like_city), NGO.state.ilike(like_city)))
        else:
            query = query.filter(NGO.city.ilike(like_city))

    ngos = query.order_by(NGO.organization_name.asc()).paginate(page=page, per_page=per_page, error_out=False)
    # If user navigates past the last page (e.g., filters changed), redirect to the last valid page
    try:
        pages = ngos.pages or 0
    except Exception:
        pages = 0
    if page > 1 and ((pages == 0) or (page > pages)):
        target = 1 if pages == 0 else pages
        return redirect(url_for('ngos_directory', q=search_term or '', category=category or '', city=city or '', page=target))
...
def volunteer_event_detail(event_id: int):
    ...
    # Include all slots so UI can indicate 'Ended' vs available
    time_slots = TimeSlot.query.filter_by(event_id=event.id).order_by(TimeSlot.start_time.asc()).all()
    # Provide current IST time (naive) for template comparisons
    return render_template('volunteer/event_detail.html', event=event, ngo=ngo, time_slots=time_slots, now_ts=datetime.now(ZoneInfo("Asia/Kolkata")).replace(tzinfo=None))
...
def ngo_events():
    ...
    query = Event.query.filter_by(ngo_id=ngo.id)
    if q:
        like = f"%{q}%"
        query = query.filter(or_(Event.title.ilike(like), Event.description.ilike(like), Event.location.ilike(like)))
    if status == 'active':
        query = query.filter(getattr(Event, 'is_active', True) == True)
    elif status == 'inactive':
        query = query.filter(getattr(Event, 'is_active', True) == False)
    if category:
    ...
    try:
        if date_from:
            ...
            query = query.filter(Event.start_date >= df)
        if date_to:
    ...
    events = query.order_by(Event.created_at.desc()).paginate(page=page, per_page=9, error_out=False)
    return render_template('ngo/events.html', events=events, ngo=ngo, q=q, status=status, category=category, date_from=date_from, date_to=date_to)


# ===== QR Attendance: NGO QR Page, Token APIs, Volunteer Check-in =====
def _now_utc():
    # Return IST-naive current time for consistent comparisons/storage
    try:
        return datetime.now(ZoneInfo("Asia/Kolkata")).replace(tzinfo=None)
    except Exception:
        return datetime.now().replace(tzinfo=None)

# ===== Automatic maintenance: deactivate ended events/slots and complete bookings =====
def _run_maintenance_if_due():
    try:
        last = app.config.get('LAST_MAINTENANCE_AT')
        now = _now_utc()
        # Run at most once every 10 minutes
        if last and (now - last) < timedelta(minutes=10):
            return

        # Deactivate time slots that have ended
        try:
        ...
        # Mark events as completed/inactive if their end_date has passed
        try:
            expired_events = Event.query.filter(
                getattr(Event, 'is_active', True) == True,
                Event.end_date < now
            ).all()
        ...
        # Auto checkout and finalize hours for bookings whose slot has ended
        try:
            ended = (db.session.query(Booking, TimeSlot)
                     .join(TimeSlot, Booking.time_slot_id == TimeSlot.id)
                     .filter(TimeSlot.end_time < now)
                     .all())
            for b, ts in ended:
                # Mark absences
                if not b.attendance_status or b.attendance_status == 'pending':
                    b.attendance_status = 'absent'
                # If attended but not checked out, auto-checkout at slot end and finalize
                if b.attendance_status == 'present' and b.check_in_at and not b.check_out_at:
                    b.check_out_at = ts.end_time
                    _finalize_booking_hours(b, ts)
            db.session.commit()
        except Exception:
            db.session.rollback()

        db.session.commit()
        app.config['LAST_MAINTENANCE_AT'] = now
    except Exception:
        db.session.rollback()
        # Do not raise; maintenance should be best-effort
        pass

@app.before_request
def _maintenance_hook():
    # Lightweight, throttled housekeeping on incoming requests
    _run_maintenance_if_due()

# ---- Attendance checkout (manual) ----
def _finalize_booking_hours(booking: Booking, ts: TimeSlot | None):
    try:
        # Use check-in/out and cap within slot window when available
        if not booking.check_in_at or not booking.check_out_at:
            return
        start_eff = booking.check_in_at
        end_eff = booking.check_out_at
        if ts:
            if ts.start_time:
                start_eff = max(start_eff, ts.start_time)
            if ts.end_time:
                end_eff = min(end_eff, ts.end_time)
        dur_sec = max(0, int((end_eff - start_eff).total_seconds()))
        dur_hours = round(dur_sec / 3600.0, 2)
        booking.hours_worked = max(booking.hours_worked or 0.0, dur_hours)
        booking.points_earned = max(booking.points_earned or 0, int(round(dur_hours * 10)))
        booking.status = 'completed'
        # Update aggregates on volunteer
        try:
            vol = db.session.get(Volunteer, booking.volunteer_id)
            if vol:
                vol.total_hours = (vol.total_hours or 0) + booking.hours_worked
                vol.total_points = (vol.total_points or 0) + booking.points_earned
        except Exception:
            pass
    except Exception:
        pass
...
def api_attendance_checkout():
    ...
    try:
        ...
        # Allow checkout if token has not expired too long; if expired, still allow but warn
        if at.expires_at < _now_utc():
            # Soft-expired; proceed to allow manual checkout
            pass
        volunteer = Volunteer.query.filter_by(user_id=current_user.id).first()
        if not volunteer:
        ...
        if not booking.check_in_at:
            return jsonify({'error': 'You have not checked in for this event/time slot'}), 400
        # Mark checkout and finalize
        booking.check_out_at = _now_utc()
        ts = db.session.get(TimeSlot, booking.time_slot_id) if booking.time_slot_id else None
        _finalize_booking_hours(booking, ts)
        db.session.commit()
        # After checkout, redirect to event summary or dashboard
        try:
            redirect_url = url_for('volunteer_event_detail', event_id=booking.event_id)
        except Exception:
            redirect_url = url_for('volunteer_dashboard')
        return jsonify({'message': 'Check-out recorded', 'booking_id': booking.id, 'hours': booking.hours_worked, 'points': booking.points_earned, 'redirect': redirect_url})
    except Exception as e:
        db.session.rollback()
...
@app.route('/volunteer/scan-qr')
@login_required
def volunteer_scan_qr():
    # Volunteers can open camera to scan attendance QR shown by NGO
    if current_user.role != 'volunteer':
        flash('Access denied. Volunteer privileges required.', 'error')
        return redirect(url_for('dashboard'))
...
def api_attendance_checkin():
    ...
    try:
        ...
        if not booking:
            return jsonify({'error': 'No matching booking found for this event/time slot'}), 404
        # Mark check-in only; finalization occurs on checkout or auto after slot end
        now_ts = _now_utc()
        booking.check_in_at = now_ts
        booking.attendance_status = 'present'
        db.session.commit()
        # Provide redirect to event details so volunteer sees work details
        try:
            redirect_url = url_for('volunteer_event_detail', event_id=booking.event_id)
        except Exception:
            redirect_url = url_for('volunteer_dashboard')
        return jsonify({'message': 'Check-in recorded', 'booking_id': booking.id, 'check_in_at': booking.check_in_at.isoformat(), 'redirect': redirect_url})
    except Exception as e:
        db.session.rollback()
...
def ngo_create_event():
    ...
    if request.method == 'POST':
        try:
            ...
            # Create manual time slots from form arrays (optional)
            # Accept both names with and without [] suffix to be resilient to template variants
            slot_dates = request.form.getlist('slot_date[]') or request.form.getlist('slot_date')
            slot_starts = request.form.getlist('slot_start[]') or request.form.getlist('slot_start')
            slot_ends = request.form.getlist('slot_end[]') or request.form.getlist('slot_end')
            slot_caps = request.form.getlist('slot_capacity[]') or request.form.getlist('slot_capacity')

            # Only attempt to create time slots if at least one date/start/end is provided
            if slot_dates or slot_starts or slot_ends or slot_caps:
                if not (slot_dates and slot_starts and slot_ends and slot_caps):
                    raise ValueError('Time slot inputs are incomplete. Please review your slots.')
    ...
    return render_template('ngo/event_form.html', mode='create', ngo=ngo, event=None)
...
def ngo_view_event(event_id):
    ...
    return render_template('ngo/event_view.html', event=event, time_slots=time_slots, bookings=bookings, ngo=ngo, now_ts=datetime.utcnow())
...
def ngo_edit_event(event_id):
    ...
    return render_template('ngo/event_form.html', mode='edit', event=event, ngo=ngo)
...
def api_my_bookings():
    if current_user.role != 'volunteer':
        return jsonify({'error': 'Only volunteers can view their bookings'}), 403
    try:
        volunteer = Volunteer.query.filter_by(user_id=current_user.id).first()
        if not volunteer:
            return jsonify({'error': 'Volunteer profile not found'}), 400
        rows = (db.session.query(Booking, Event, TimeSlot)
                .join(Event, Booking.event_id == Event.id, isouter=True)
                .join(TimeSlot, Booking.time_slot_id == TimeSlot.id, isouter=True)
                .filter(Booking.volunteer_id == volunteer.id)
                .order_by(Booking.created_at.desc())
        ...
        for b, e, ts in rows:
            out.append({
                'booking_id': b.id,
                'event_id': b.event_id,
                'event_title': getattr(e, 'title', None) or getattr(e, 'name', None),
...
# Inline toggle of user active status (AJAX-friendly)
...
def admin_set_user_active(user_id: int):
    try:
        ...
        active_raw = (request.form.get('active') or '').lower()
        new_active = True if active_raw in ('1','true','yes','on') else False
        user.is_active = new_active
        db.session.commit()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({ 'success': True, 'id': user.id, 'is_active': user.is_active })
...
def admin_clear_audit_logs():
    try:
        mode = request.form.get('mode')  # 'older_than' or 'range'
        deleted = 0
        if mode == 'older_than':
            cutoff = request.form.get('cutoff_date')  # YYYY-MM-DD
            if not cutoff:
                raise ValueError('cutoff_date is required')
            cutoff_dt = datetime.strptime(cutoff, '%Y-%m-%d')
            deleted = AdminAuditLog.query.filter(AdminAuditLog.timestamp < cutoff_dt).delete(synchronize_session=False)
        elif mode == 'range':
            ...
            dt_dt = datetime.strptime(dt, '%Y-%m-%d') + timedelta(days=1)
            deleted = AdminAuditLog.query.filter(AdminAuditLog.timestamp >= df_dt, AdminAuditLog.timestamp < dt_dt).delete(synchronize_session=False)
        else:
            raise ValueError('Invalid mode')

        db.session.commit()
        log_admin_action('CLEAR_AUDIT_LOGS', 'AUDIT_LOG', details={'mode': mode, 'deleted': deleted})
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({'success': True, 'deleted': deleted})
        flash(f'Deleted {deleted} audit log(s).', 'success')
        return redirect(request.referrer or url_for('admin_audit_logs'))
    except Exception as e:
        db.session.rollback()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
...
@app.route('/admin/audit-logs/clear/preview', methods=['GET'])
@admin_required
@admin_permission_required('view_audit_logs')
def admin_clear_audit_logs_preview():
    try:
        mode = request.args.get('mode')
        if mode == 'older_than':
            cutoff = request.args.get('cutoff_date')
            if not cutoff:
                return jsonify({'error': 'cutoff_date is required'}), 400
            cutoff_dt = datetime.strptime(cutoff, '%Y-%m-%d')
            count = db.session.query(AdminAuditLog).filter(AdminAuditLog.timestamp < cutoff_dt).count()
        elif mode == 'range':
            ...
            df_dt = datetime.strptime(df, '%Y-%m-%d')
            dt_dt = datetime.strptime(dt, '%Y-%m-%d') + timedelta(days=1)
            count = db.session.query(AdminAuditLog).filter(AdminAuditLog.timestamp >= df_dt, AdminAuditLog.timestamp < dt_dt).count()
        else:
...
# Inline toggle of NGO verification (AJAX-friendly)
@app.route('/admin/ngos/<int:ngo_id>/set-verified', methods=['POST'])
@admin_required
@admin_permission_required('manage_ngos')
def admin_set_ngo_verified(ngo_id: int):
    try:
        ngo = NGO.query.get_or_404(ngo_id)
        verified_raw = (request.form.get('verified') or '').lower()
        new_verified = True if verified_raw in ('1','true','yes','on') else False
        ngo.is_verified = new_verified
        db.session.commit()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({'success': True, 'id': ngo.id, 'is_verified': ngo.is_verified})
        flash('NGO verification updated', 'success')
    except Exception as e:
        db.session.rollback()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({'success': False, 'error': str(e)}), 500
        flash(f'Failed to update NGO: {str(e)}', 'error')
...
# Inline toggle of Event active status (AJAX-friendly)
@app.route('/admin/events/<int:event_id>/set-active', methods=['POST'])
@admin_required
@admin_permission_required('manage_events')
def admin_set_event_active(event_id: int):
    try:
        event = Event.query.get_or_404(event_id)
        active_raw = (request.form.get('active') or '').lower()
        new_active = True if active_raw in ('1','true','yes','on') else False
        event.is_active = new_active
        db.session.commit()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({'success': True, 'id': event.id, 'is_active': event.is_active})
        flash('Event status updated', 'success')
    except Exception as e:
        db.session.rollback()
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
            return jsonify({'success': False, 'error': str(e)}), 500
        flash(f'Failed to update event: {str(e)}', 'error')
