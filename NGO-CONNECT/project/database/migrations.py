...
class DatabaseMigration:
    ...
    def _register_migrations(self):
        ...
        self.migrations = [
            ...
            {
                'version': 7,
                'name': 'Add booking attendance columns',
                'description': 'Add check_in_at, check_out_at, attendance_status to bookings table',
                'sql': '''
                -- Add attendance-related columns to bookings if they don't exist
                SET @dbname = DATABASE();
                SET @tablename = 'bookings';

                -- check_in_at
                SET @columnname = 'check_in_at';
                SET @preparedStatement = (SELECT IF(
                  (
                    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
                ...
                -- attendance_status
                SET @columnname = 'attendance_status';
                SET @preparedStatement = (SELECT IF(
                  (
                    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
                    WHERE (TABLE_NAME = @tablename) AND (TABLE_SCHEMA = @dbname) AND (COLUMN_NAME = @columnname)
                  ...
                  'SELECT 1',
                  CONCAT('ALTER TABLE ', @tablename, " ADD COLUMN ", @columnname, " VARCHAR(20) DEFAULT 'pending';")
                ));
                PREPARE alterIfNotExists FROM @preparedStatement;
                EXECUTE alterIfNotExists;
                DEALLOCATE PREPARE alterIfNotExists;
        ...
        try:
            with app.app_context():
                ...
                if migration['version'] == 3:
                    ...
                    # Add attendance-related columns to bookings if they don't exist
                    if not self.column_exists('bookings', 'check_in_at'):
                        db.session.execute(text("ALTER TABLE bookings ADD COLUMN check_in_at DATETIME NULL"))
                        print("  Added check_in_at column to bookings table")
                    if not self.column_exists('bookings', 'check_out_at'):
                        db.session.execute(text("ALTER TABLE bookings ADD COLUMN check_out_at DATETIME NULL"))
                        print("  Added check_out_at column to bookings table")
                    if not self.column_exists('bookings', 'attendance_status'):
                        db.session.execute(text("ALTER TABLE bookings ADD COLUMN attendance_status VARCHAR(20) DEFAULT 'pending'"))
                        print("  Added attendance_status column to bookings table")
